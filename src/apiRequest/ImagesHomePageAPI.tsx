...
export const ImagesHomePageAPI = {
  getImagesHomePage(body: ImagesHomePageBodyType) {
    return httpRequest.get<ImagesHomePageResType>(
      `${process.env.NEXT_PUBLIC_API_ENDPOINT}/imageshomepage?storeId=${body.storeId}`,
      {
        cache: "no-cache",
      }
    );
  ...
  createImagesHomePage(body: CreateImagesHomePageBodyType) {
    return httpRequest.post<CreateImagesHomePageResType>(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/imageshomepage`, {
      body,
      credentials: "include",
    });
  },
  updateImagesHomePage(body: UpdateImagesHomePageBodyType) {
    return httpRequest.put<UpdateImagesHomePageResType>(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/imageshomepage`, {
      body,
      credentials: "include",
    });
  },
